...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getConvModal() {
    ...
    if (!m) {
      ...
      m.addEventListener("click", (ev) => {
        if (ev.target === m) settingsModule.closeModal("_convModalDyn");
      });
    }
    return m;
  }
  ...
  window._patchToggleFixedMod = function (btn, key) {
    const hiddenEl = document.getElementById(
      `_mod${key.charAt(0).toUpperCase() + key.slice(1)}`,
    );
  ...
  settingsModule.openConvModal = function (id) {
    ...
    const statusOpts = [
      ...
      .map(
        (o) =>
          `<option value="${o.v}" ${String(cv?.is_active ?? "1") === o.v ? "selected" : ""}>${o.l}</option>`,
      )
      .join("");

    const pricingOpts = [
      { v: "fixed", l: "Precio fijo" },
      { v: "staged", l: "Precio por etapas" },
    ]
      .map(
        (o) =>
          `<option value="${o.v}" ${pricingMode === o.v ? "selected" : ""}>${o.l}</option>`,
      )
      .join("");
            ...
            <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
              ...
              ${
                ...
                <i class="fas fa-upload"></i> Subir / Reemplazar PDF
                  ...
                  : '<small style="color:var(--text-mute)">Guarda la convocatoria primero para subir el PDF.</small>'
              }
            </div>
          </div>
        </div>
  ...
  window._patchSaveConv = async function () {
    ...
    if (!titulo)
      return settingsModule.toast("El título es obligatorio", "error");
    ...
    if (mode === "fixed") {
      ...
      if (isNaN(precio_base))
        return settingsModule.toast("Ingresa un precio válido", "error");
    } else {
      const rows = document.querySelectorAll(
        "#_convPriceStages .price-stage-row",
      );
      const stages = [];
      for (const row of rows) {
        ...
        if (!start || !end || isNaN(price))
          return settingsModule.toast(
            "Completa todas las etapas de precio",
            "error",
          );
    ...
    const customFixedRaw =
      document.getElementById("_modCustomFixedData")?.textContent || "[]";
    let customFixed = [];
    try {
      customFixed = JSON.parse(customFixedRaw);
    } catch {}

    const included_modules = JSON.stringify({
      congress,
      robotics,
      camp,
      custom: customFixed,
  ...
  settingsModule.openModuleModal = function (convId, moduleId) {
    ...
    const typeOpts = [
      ...
      .map(
        (o) =>
          `<option value="${o.v}" ${(mod?.module_type || "workshop") === o.v ? "selected" : ""}>${o.l}</option>`,
      )
      .join("");
    ...
    const roleOpts = [
      ...
      .map(
        (o) =>
          `<option value="${o.v}" ${(mod?.responsible_role || "instructor") === o.v ? "selected" : ""}>${o.l}</option>`,
      )
      .join("");
    ...
    const statusOpts = [
      ...
      .map(
        (o) =>
          `<option value="${o.v}" ${(mod?.status || "draft") === o.v ? "selected" : ""}>${o.l}</option>`,
      )
      .join("");
    ...
    const configVal = mod?.config_json
      ? JSON.stringify(mod.config_json, null, 2)
      : "";
  ...
  window._patchSaveModule = async function () {
    ...
    const icon =
      document.getElementById("_mmIcon").value.trim() || "fas fa-star";
    ...
    if (cfgRaw) {
      try {
        configJson = JSON.stringify(JSON.parse(cfgRaw));
      } catch {
        return settingsModule.toast(
    ...
    const moduleKey = `${type}_${title
      .toLowerCase()
      .replace(/\s+/g, "-")
      .replace(/[^a-z0-9-]/g, "")}_${moduleId || Date.now()}`;
    ...
    if (org || bio || cfgRaw)
      fd.append("config_json", JSON.stringify(extraConfig));
  ...
  window._patchPickResponsiblePhoto = function (convId, modId) {
    if (!modId) {
      settingsModule.toast(
        "Guarda el módulo primero para subir la foto del responsable.",
        "info",
      );
      return;
    }
    ...
    input.onchange = async (ev) => {
      ...
      if (file.size > 5 * 1024 * 1024) {
        settingsModule.toast("La imagen supera 5 MB", "error");
        return;
      }
      const fd = new FormData();
      ...
      try {
        ...
        const thumb =
          document.getElementById("_respPhotoThumb") ||
          document.querySelector(".resp-photo-thumb");
        if (thumb) {
  ...
  window._patchPickModuleImage = function (convId, modId) {
    if (!modId) {
      settingsModule.toast(
        "Guarda el módulo primero para subir imágenes de galería.",
        "info",
      );
      return;
    }
    ...
    input.onchange = async (ev) => {
      ...
      for (const file of files) {
        if (file.size > 5 * 1024 * 1024) {
          settingsModule.toast("Una imagen supera 5 MB", "error");
          continue;
        }
        const fd = new FormData();
  ...
  console.log(
    "[admin-settings-patch] ✓ openConvModal y openModuleModal actualizados",
  );
});
