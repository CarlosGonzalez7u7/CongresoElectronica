/**
 * patch-conferencias-usuario.js
 * 
 * Parche para usuario.js / usuario.html:
 *  1. Las tarjetas de conferencias ahora muestran botón de inscripción igual que talleres.
 *  2. Al abrir el modal de una conferencia, el botón inscribirse/darse de baja aparece
 *     condicionado al pago de la convocatoria correcta (no solo "tiene congreso").
 *
 * Cargar al final de <body> DESPUÉS de usuario.js:
 *   <script src="js/patch-conferencias-usuario.js?v=1"></script>
 */

(function () {
  'use strict';

  // ── Helpers reutilizados de usuario.js ──────────────────────────────────────
  const esc = (s) => {
  ...
  // ── Estado de conferencias del usuario ──────────────────────────────────────
  // Reutilizamos userEnrolledConferenceIds de usuario.js (ya es global window-scope).
  // Añadimos paid_convocatorias de la respuesta de conference-enroll.php.
  let _confPaidConvs = [];
  ...
  async function refreshConferenceState() {
    ...
    try {
      ...
      if (res.success) {
        ...
        window.userEnrolledConferenceIds = (res.enrolled_conference_ids || []).map(Number);
      }
    } catch (e) {
      console.warn('[patch-conf] No se pudo refrescar estado de conferencias', e);
    }
  }
  ...
  window.patchInscribirConferencia = async function (conferenceId) {
    ...
    const session = getSession();
    if (!session) { alert('Debes iniciar sesión primero.'); return; }
    ...
    try {
      const res = await fetch('/app/api/conference-enroll.php', {
        ...
        body: JSON.stringify({ action: 'enroll', conferenceId, userId: session.id || session.userId || session.user_id }),
      }).then(r => r.json());
      ...
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-user-plus"></i> Inscribirme a esta conferencia';
      }
    }
  };
  ...
  window.patchDarBajaConferencia = async function (conferenceId) {
    ...
    try {
      const res = await fetch('/app/api/conference-enroll.php', {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'unenroll', conferenceId, userId: session.id || session.userId || session.user_id }),
      }).then(r => r.json());
  ...
  window.mostrarDetalleConferencia = function (id) {
    ...
    if (!c) {
      if (_originalMostrarDetalleConferencia) _originalMostrarDetalleConferencia(id);
      return;
    }

    const isEnrolled = (window.userEnrolledConferenceIds || []).includes(Number(c.id));
    ...
    let tags = [];
    try { tags = typeof c.tags === 'string' ? JSON.parse(c.tags) : (c.tags || []); } catch { tags = []; }
    ...
    const capacityHtml = c.capacity > 0
