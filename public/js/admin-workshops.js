...
const conferencesModule = (function () {
  ...
  async function openCallForSpeakersModal() {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        document.getElementById("cfs-phone").value = d.phone || "";
        document.getElementById("cfsDocsJson").value = JSON.stringify(
          d.docs || [],
        );
        if (typeof renderNativeDocs === "function") renderNativeDocs("cfs");
      }
    } catch (e) {}
  }
  ...
  async function saveCallForSpeakers() {
    const data = {
      ...
      description: document.getElementById("cfs-desc").value.trim(),
      email: document.getElementById("cfs-email").value.trim(),
      phone: document.getElementById("cfs-phone").value.trim(),
      docs: JSON.parse(document.getElementById("cfsDocsJson").value || "[]"),
    };
    try {
      const res = await wsApi("", "POST", {
        action: "save_call_for_speakers",
        data,
      });
      if (res.success) {
