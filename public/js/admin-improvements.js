...
(function patchScanner() {
  document.addEventListener("DOMContentLoaded", () => {
    if (document.getElementById("scanner-fullscreen-css")) return;
    const style = document.createElement("style");
    style.id = "scanner-fullscreen-css";
    style.textContent = `
      .scanner-box {
      ...
      .admin-scanner-video {
        ...
        border: 2px solid rgba(0, 198, 248, 0.4) !important;
...
(function patchHistoryPanel() {
  ...
  function patchWhenReady() {
    ...
    window.renderHistoryPanel = function renderHistoryPanelImproved(teams) {
      ...
      const ordered =
        typeof sortTeamsByStageThenDate === "function"
    ...
    window.applyHistoryFilters = function applyHistoryFiltersImproved() {
      ...
      if (statusValue !== "all") {
        filtered = filtered.filter((team) => {
          const arrival =
            typeof getArrivalSummary === "function"
              ? getArrivalSummary(team)
              : { status: "missing" };
      ...
      if (searchValue) {
        filtered = filtered.filter((team) => {
          const robots = (team.robots || [])
            ...
            .join(" ");
          const text =
            `${team.folio || ""} ${team.captain_name || ""} ${team.school_name || ""} ${robots}`.toLowerCase();
    ...
    const html = slice
      .map((team) => {
        const arrival =
          typeof getArrivalSummary === "function"
            ? getArrivalSummary(team)
            : { status: "missing", label: "Sin info", detail: "-" };
        const stage =
          typeof getStageDefinition === "function"
            ? getStageDefinition(team.registration_stage)
            : { shortLabel: "-" };
        const stageState =
          typeof getStageState === "function"
            ? getStageState(team.registration_stage)
            : "";
        const robots = team.robots || [];

        const robotChips = robots
          .map((r) => {
        ...
        const paymentBadge =
          typeof mapPaymentBadge === "function"
            ? mapPaymentBadge(team.payment_status).text
            : team.payment_status || "-";
        const checkinStr = team.checkin_at
          ? typeof formatDateTime === "function"
            ? formatDateTime(team.checkin_at)
            : team.checkin_at
          : "-";
  ...
  function renderHistoryPagination(total, totalPages) {
    ...
    if (totalPages <= maxBtns) {
      ...
      pages = [1];
      const around = [_historyPage - 1, _historyPage, _historyPage + 1].filter(
        (p) => p > 1 && p < totalPages,
      );
    ...
    pag.querySelector("#histPrev")?.addEventListener("click", () => {
      _historyPage--;
      renderHistoryPage();
    });
    pag.querySelector("#histNext")?.addEventListener("click", () => {
      _historyPage++;
      renderHistoryPage();
    });
...
(function patchSecurityPanel() {
  ...
  function patchWhenReady() {
    ...
    window.renderSecurityActivityTable =
      function renderSecurityTableImproved() {
  ...
  function renderSecPage() {
    ...
    if (!slice.length) {
      body.innerHTML =
        '<tr><td colspan="7" class="sec-table-empty"><i class="fas fa-inbox"></i><br>Sin actividad para los filtros seleccionados.</td></tr>';
  ...
  function renderSecPagination(total, totalPages) {
    ...
    if (totalPages <= maxBtns) {
      ...
      pages = [1];
      const around = [_secPage - 1, _secPage, _secPage + 1].filter(
        (p) => p > 1 && p < totalPages,
      );
      if (around[0] > 2) pages.push("…");
      pages = pages.concat(around);
    ...
    btnsContainer.querySelector("#secPrev")?.addEventListener("click", () => {
      _secPage--;
      renderSecPage();
    });
    btnsContainer.querySelector("#secNext")?.addEventListener("click", () => {
      _secPage++;
      renderSecPage();
    });
  ...
  function initSecurityDeleteBtn() {
    ...
    btn.addEventListener("click", async () => {
      ...
      if (
        !confirm(
          `¿Borrar registros de actividad de los ${label}?\nEsta acción no se puede deshacer.`,
        )
      )
        return;
      ...
      rangeEl.value = "";
      setSecMsg(
        `Se eliminaron ${removed} registro${removed !== 1 ? "s" : ""} de los ${label}.`,
        "success",
      );
    });
  }
...
(function initBackupSystem() {
  ...
  function renderBackupHistory() {
    ...
    if (!history.length) {
      container.innerHTML =
        '<p style="font-size:0.75rem; color:var(--text-mute); margin-top:6px;">Sin respaldos previos en esta sesión.</p>';
      return;
    }
  ...
  async function generateBackup(type = "manual") {
    ...
    try {
      ...
      if (typeof setSecurityMessage === "function") {
        setSecurityMessage(
          `Respaldo generado: ${filename} (${sizeStr})`,
          "success",
        );
      }
    } catch (err) {
      if (typeof setSecurityMessage === "function") {
        setSecurityMessage(
          `Error al generar respaldo: ${err.message}`,
          "error",
        );
      }
    } finally {
        ...
        if (typeof setSecurityMessage === "function") {
          setSecurityMessage(
            current
              ...
              : "Respaldo automático activado. Se ejecutará cada 24h mientras el panel esté abierto.",
            "success",
          );
        }
      });
